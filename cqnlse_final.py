...
class SCI_Figure_Generator:
    ...
    def fig6_pdf(self):
        ...
        """
        s  = self.s
        n  = s.I_hist.shape[1]; i0 = max(1, int(n*0.25))
...
def alpha_sensitivity_scan(base_params):
    ...
    for al, col, lab in zip(traj_alphas, traj_colors, traj_labels):
        ...
        nz_t = int(round(p_t.z_max/p_t.dz))+1; rec_t = max(1, nz_t//400)
        zz, mx = [], []
        for i in range(nz_t):
...
def print_report(params, solver):
    ...
    print("="*68 + f"{ENDC}")
    print(f"  {BOLD}Physical Params:{ENDC} beta2={p.beta2:+.1f}, gamma={p.gamma:.1f}, alpha={p.alpha:.3f}, A0={p.A0:.2f}")
    ...
    ai_status = f"{FAIL}→ EXTREME EVENT REGIME (AI > 2.0){ENDC}" if st['AI'] > 2.0 else f"{GREEN}→ Below RW Threshold{ENDC}"
    print(f"    Abnormality Index (AI)      = {BOLD}{st['AI']:.4f}{ENDC}  {ai_status}")
    
    kurt_status = f"{WARN}→ Heavy-Tailed (Non-Gaussian){ENDC}" if st['kurtosis'] > 3.0 else f"{GREEN}→ Near Gaussian{ENDC}"
    ...
    h_err = st['max_hamiltonian_error']
    
    p_grade = f"{GREEN}PASS (Machine Precision){ENDC}" if p_err < 1e-12 else f"{FAIL}FAIL{ENDC}"
    h_grade = f"{GREEN}PASS (O(dz²) Verified){ENDC}" if h_err < 1e-4 else f"{WARN}WARN (Check dz){ENDC}"
    
    print(f"    Max Power Error |ΔP/P₀|     = {p_err:.2e}  {p_grade}")
    print(f"    Hamiltonian Error |ΔH/H₀|   = {h_err:.2e}  {h_grade}")
    print(f"{BLUE}" + "="*68 + f"{ENDC}\n")
